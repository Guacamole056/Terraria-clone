...
buttonclass::buttonclass() { position = {0, 0}; }
...
void buttonclass::update() {
  ...
  for (int i = 0; i < nroftiles; i++) {
    if (i == 0) {
      ...
      inventory_meniu_sprite.setPosition(position.x + i * 32 * scale + XVIEW,
                                         position.y + YVIEW);
      window.draw(inventory_meniu_sprite);
    } else if (i == nroftiles - 1) {

      inventory_meniu_sprite.setRotation(180);

      changetexture(inventory_meniu_sprite, 5, 0, 0);
      inventory_meniu_sprite.setPosition(position.x + (i + 1) * 32 * scale +
                                             XVIEW,
                                         position.y + 1 * 32 * scale + YVIEW);
      window.draw(inventory_meniu_sprite);
  ...
  if (mousepos.x > position.x && mousepos.y > position.y &&
      ...
      mousepos.y < position.y + 32 * scale && leftreleased) {
    onclick();
  }
}
