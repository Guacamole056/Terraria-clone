...
void makemove(humanlikeenemy &ent, entities other) {
    ...
    if (abs(ent.x - other.x) < ent.lineofsight * 32) {
      ...
      if (other.y + 64 < ent.y) /// jump to player
      {
        if (ent.canjump && ent.jumpwhaittime <= 0) {
          ent.velocity.y = -sqrtf(2.0f * 9.81 * 32 * ent.jumpheight.x);
          ent.canjump = 0;
      ...
      if (ent.moving)
        ...
        {
          if (world[ent.x / 32 + 1][(ent.y + ent.sizeofentity.y) / 32]
                  .family() == bl::varation::fragile)
            if (ent.canjump) {
              ent.velocity.y = -sqrtf(2.0f * 9.81 * 32 * ent.jumpheight.x);
              ent.canjump = 0;
            }
      ...
      if (world[ent.x / 32 + 1][ent.y / 32].family() ==
              bl::varation::blocy || /// jump over walls
          world[ent.x / 32 - 1][ent.y / 32].family() == bl::varation::blocy ||
          world[ent.x / 32 + 1][ent.y / 32 + 1].family() ==
              bl::varation::blocy ||
  ...
  if (!ent.canjump) {
    ent.animationstate = humanlikeenemy::jumping;
  }
}
...
void makemove(slimeentity &ent, entities other) {
  ...
  if (ent.frosttime <= 0) {
    ...
    if (ent.timetillnextmove > ent.whaittime) {
      ...
      if (ent.canjump) {
        ent.velocity.y = -sqrtf(
            2.0f * 9.81 * 32 * randomrange(ent.jumpheight.x, ent.jumpheight.y));
      }
      ...
      if (std::abs(other.x - ent.x) < 32 * ent.lineofsight) // check player
      {
        ...
        ent.whaittime = randomrange(8, 12) / 2.f;
      }
    }
    ...
    if (!ent.canjump) {
      ...
      if (ent.movingright) {
        ent.velocity.x += ent.tempmovespeed;
      } /// move randomly
      else {
        ent.velocity.x -= ent.tempmovespeed;
      }
