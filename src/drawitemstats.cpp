    ...
    if (rec.type) {
      ...
      if (!rec.placeble) {
        ...
        if (item->getattackpower()) {
          if (item->getspeed() == 4) {
            text.setString("Very slow speed");
          } else if (item->getspeed() == 3) {
            text.setString("Slow speed");
          } else if (item->getspeed() == 2) {
            text.setString("Average sped");
          } else if (item->getspeed() == 1) {
            text.setString("Fast sped");
          } else {
            text.setString("Insanely fast sped");
          }

          if (item->gethammerpower() || item->getpickaxepower() ||
              item->getaxepower()) {
            text.setPosition(13 * 64 + xview, 2 * 64 + yview);
          } else {
        ...
        if (item->getarmour() || item->getfalldamagerezistance()) {
          ...
          if (item->getfalldamagerezistance() >= 0) {
            ...
            text.setString(text.getString() +
                           inttostring(item->getfalldamagerezistance()));
          } else {
        ...
        if (item->getaxepower()) {
          ...
          text.setString(text.getString() +
                         inttostring(item->getpickaxepower()));
          text.setPosition(13 * 64 + xview, 2.5 * 64 + yview);
          window.draw(text);
        } else if (item->gethammerpower()) {
          text.setString("Hammer power ");
          text.setString(text.getString() +
                         inttostring(item->gethammerpower()));
          text.setPosition(13 * 64 + xview, 2.5 * 64 + yview);
          window.draw(text);
        }
        ...
        if (item->getattackpower()) {
          ...
          text.setString(text.getString() +
                         inttostring(item->getattackpower()));
