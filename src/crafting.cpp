...
void crafting() {
  ...
  /// draw items
  if (leftreleased && getbutton(pointingto) >= 0) {
    selecteditem = getbutton(pointingto) + pedding;
  } /// buttons
  if (leftreleased && getbutton(pointingto) == -3) {
    if (pedding)
      pedding--;
  } /// buttons
  if (leftreleased && getbutton(pointingto) == -4) {
    if (pedding + 5 < nrofrecepies)
      pedding++;
  } /// buttons

  if (leftreleased && getbutton(pointingto) == -6) {
    pedding = 0;
  }
  if (leftreleased && getbutton(pointingto) == -7) {
    pedding = 23;
  } /// quick buttons
  if (leftreleased && getbutton(pointingto) == -8) {
    pedding = 59;
  }
  if (leftreleased && getbutton(pointingto) == -9) {
    pedding = 70;
  }
  if (leftreleased && getbutton(pointingto) == -10) {
    pedding = 86;
  }
  ...
  for (int i = pedding; i < pedding + 5; i++) /// draw result
  {
    ...
    inventory_meniu_sprite.setPosition(7 * 64 + xview,
                                       (i + 2 - pedding) * 64 + yview);
    window.draw(inventory_meniu_sprite);
    ...
    for (int ii = 0; ii < 4; ii++) /// draw materials
    {
      if (recepies[i].materials[ii].type) {
        ...
        if (recepies[i].materials[ii].placeble) {
          ...
          blocks_sprite.setPosition((3 + ii) * 64 + xview,
                                    (i + 2 - pedding) * 64 + yview);
          window.draw(blocks_sprite);
        } else {
          changetexture(items_sprite, recepies[i].materials[ii].type, 0);
          items_sprite.setPosition((3 + ii) * 64 + xview,
                                   (i + 2 - pedding) * 64 + yview);
          window.draw(items_sprite);
        }
      }
      ...
      if (recepies[i].materials[ii].number > 1) {
        text.setPosition((3 + ii) * 64 + xview,
                         (i + 2.5 - pedding) * 64 + yview);
  ...
  changetexture(inventory_meniu_sprite, inventory_sprites::grey,
                0); /// craft button
  ...
  for (int i = 0; i < INVENTORY_SIZE; i++) {
    for (int ii = 0; ii < 4; ii++) {
      if (recepies[selecteditem].materials[ii].placeble ==
              inventory[i]->placeble() &&
  ...
  if (remainingmaterials[0] <= 0 && remainingmaterials[1] <= 0 &&
      remainingmaterials[2] <= 0 && remainingmaterials[3] <= 0) {
    enoughmaterials = 1;
  }
    ...
    if (enoughspace && enoughmaterials && crafingstations) {
      ...
      for (int i = 0; i < INVENTORY_SIZE; i++) {
        if (inventory[i]->placeble() ==
                recepies[selecteditem].itemcreated.placeble &&
            inventory[i]->type == recepies[selecteditem].itemcreated.type) {
          if (inventory[i]->number + remainingitems <=
              inventory[i]->maxnumber) {
          ...
          if (!inventory[i]->type) {
            ...
            if (recepies[selecteditem].itemcreated.placeble) {
              inventory[i] = new normalitems(
                  recepies[selecteditem].itemcreated.type, remainingitems);
            } else {
              inventory[i] = new nonplaceble(
                  recepies[selecteditem].itemcreated.type, remainingitems);
            }
            break;
          }
  ...
  if (getbutton(pointingto) == -2 && leftreleased) {
...
static int getbutton(sf::Vector2i vect) {
  if (vect.x == 0 && vect.y == -2) {
    return -2;
  } /// inventory
  if (vect.x >= 0 && vect.x < 6 && vect.y >= 0 && vect.y < 5) {
    return vect.y;
  }
  if (vect.x >= 0 && vect.x < 6 && vect.y == -1) {
    return -3;
  }
  if (vect.x >= 0 && vect.x < 6 && vect.y == 5) {
    return -4;
  }
  if (vect.y == 5 && vect.x >= 7 && vect.x <= 8) {
    return -5;
  }
  ...
  if (vect.x == -1 && vect.y == 2) {
    return -8;
  } // quick buttons
  if (vect.x == -1 && vect.y == 3) {
    return -9;
  } // quick buttons
  if (vect.x == -1 && vect.y == 4) {
    return -10;
  } // quick buttons
...
void checkstations() {
  ...
  for (int x = a - 3; x < a + 3; x++) {
    for (int y = b - 3; y < b + 3; y++) {
      if (world[x][y].type == bl::workbench) {
        close_to_a_workbench = 1;
      } else if (world[x][y].type == bl::furnace) {
        close_to_a_furnace = 1;
      }
    }
  }
}
