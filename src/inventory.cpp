...
void inventorystate() {
  ...
  {
    if (leftreleased) {
      if (getpos(pointingto) == -3) {
        states = gamestates::crafting_state;
        return;
      }
      if (getpos(pointingto) == -4 && currentchest.x != -1) {
        states = gamestates::chest_state;
        return;
      }
      if (getpos(pointingto) == -5) {
        states = gamestates::invent;
        return;
      }
    }
  }
  ...
  if (getpos(pointingto) == -2) {
    changetexture(blocks_sprite, 0, 2);
  } else {
  ...
  {
    for (int x = 0; x < 5; x++) {
      if (pointingto.x == x && pointingto.y == y) {
        changetexture(blocks_sprite, 0, 2);
        numberofitemslotsb--;
      } else if (numberofitemslotsb > 0) {
      ...
      blocks_sprite.setPosition((32 * 3 + x * 32) * 2 + xview,
                                (32 * 1 + y * 32) * 2 + yview);
      window.draw(blocks_sprite);

      if (inventory[counting]->placeble()) {
        changetexture(blocks_sprite, inventory[counting]->type,
                      4); /// draw items
        window.draw(blocks_sprite);
      } else {
        items_sprite.setPosition((32 * 3 + x * 32) * 2 + xview,
                                 (32 * 1 + y * 32) * 2 + yview); /// draw tools
        changetexture(items_sprite, inventory[counting]->type, 0);
        window.draw(items_sprite);
      }

      if (inventory[getpos(sf::Vector2i(x, y))]->number > 1 &&
          inventory[getpos(sf::Vector2i(x, y))]->type) {
        text.setPosition((32 * 3 + x * 32) * 2 + xview,
                         (32 * 1 + y * 32 + 16) * 2 + yview);
        text.setString(
            inttostring(inventory[getpos(sf::Vector2i(x, y))]->number));
        window.draw(text);
      }
  ...
  blocks_sprite.setPosition((32 * 3 + 4 * 32) * 2 + xview,
                            (32 * 1 + 7 * 32) * 2 + yview);
  window.draw(blocks_sprite);
  ...
  if (armour_inventory[0] != 0) {
    items_sprite.setPosition((32 * 3 + 4 * 32) * 2 + xview,
                             (32 * 1 + 7 * 32) * 2 + yview);
    changetexture(items_sprite, armour_inventory[0], 0);
    window.draw(items_sprite);
  } else {
    inventory_meniu_sprite.setPosition((32 * 3 + 4 * 32) * 2 + xview,
                                       (32 * 1 + 7 * 32) * 2 + yview);
  ...
  text.setPosition((32 * 3 + 4 * 32) * 2 + xview + 16,
                   (32 * 1 + 10 * 32) * 2 + yview + 16);
  ...
  if (currentchest.x != -1 && states == gamestates::chest_state) /// draw chest
  {
    ...
    for (int y = 0; y < 4; y++) /// drawing
    {
      for (int x = 0; x < 5; x++) {
        ...
        blocks_sprite.setPosition((32 * 9 + x * 32) * 2 + xview,
                                  (32 * 1 + y * 32) * 2 + yview);
        window.draw(blocks_sprite);

        if (chestbuffer[counting]->placeble()) {
          changetexture(blocks_sprite, chestbuffer[counting]->type,
                        4); /// draw items
          window.draw(blocks_sprite);
        } else {
          items_sprite.setPosition((32 * 9 + x * 32) * 2 + xview,
                                   (32 * 1 + y * 32) * 2 +
                                       yview); /// draw tools
        ...
        if (chestbuffer[getpos(sf::Vector2i(x, y))]->number > 1 &&
            chestbuffer[getpos(sf::Vector2i(x, y))]->type) {
          text.setPosition((32 * 9 + x * 32) * 2 + xview,
                           (32 * 1 + y * 32 + 16) * 2 + yview);
          text.setString(
              inttostring(chestbuffer[getpos(sf::Vector2i(x, y))]->number));
          window.draw(text);
        }
  ...
  if (typehold->type) {
    ...
    if (typehold->placeble()) /// draw blocks on mouse
    {
      ...
      blocks_sprite.setPosition(sf::Mouse::getPosition(window).x - 32 + xview,
                                sf::Mouse::getPosition(window).y - 32 + yview);
      window.draw(blocks_sprite);
    } else { /// draw items on mouse
      changetexture(items_sprite, typehold->type, 0);
      items_sprite.setPosition(sf::Mouse::getPosition(window).x - 32 + xview,
                               sf::Mouse::getPosition(window).y - 32 + yview);
      window.draw(items_sprite);
    }
    ...
    if (typehold->number > 1) {
      ...
      text.setPosition(sf::Mouse::getPosition(window).x - 32 + xview,
                       sf::Mouse::getPosition(window).y + yview);
      window.draw(text);
    }

    if (states != gamestates::chest_state)
      drawitemstats(
          Craftebleitem(typehold->placeble(), typehold->type, typehold->number),
          typehold);
    ...
    if (typehold->wearable() == 1 || typehold->type == 0) // head
    {
      if (getpos(pointingto) == -6) {
        if (!armour_inventory[0] && typehold->type != 0) {
          ...
          typehold->type = 0;
          typehold->number = 0;
        } else if (typehold->type == 0 && armour_inventory[0] != 0) {
          delete typehold;
    ...
    if (typehold->wearable() == 2 || typehold->type == 0) // body
    {
      if (getpos(pointingto) == -7) {
        if (!armour_inventory[1] && typehold->type != 0) {
          ...
          typehold->type = 0;
          typehold->number = 0;
        } else if (typehold->type == 0 && armour_inventory[1] != 0) {
          delete typehold;
    ...
    if (typehold->wearable() == 3 || typehold->type == 0) // feet
    {
      if (getpos(pointingto) == -8) {
        if (!armour_inventory[2] && typehold->type != 0) {
          ...
          typehold->type = 0;
          typehold->number = 0;
        } else if (typehold->type == 0 && armour_inventory[2] != 0) {
          delete typehold;
    ...
    if (pointingto.x < 5) {
      ...
      if (leftreleased) {
        if (typehold->type) /// place
        {
          ...
          if (typehold->type == inventory[getpos(pointingto)]->type &&
              typehold->placeble() ==
                  inventory[getpos(pointingto)]->placeble() &&
              inventory[getpos(pointingto)]->number !=
                  inventory[getpos(pointingto)]->maxnumber) {
            if (inventory[getpos(pointingto)]->number + typehold->number <=
                inventory[getpos(pointingto)]->maxnumber) {
              ...
              typehold->number -=
                  (typehold->maxnumber - inventory[getpos(pointingto)]->number);
              inventory[getpos(pointingto)]->number =
                  inventory[getpos(pointingto)]->maxnumber; /// when filled
            }

          } else /// change
          {
            if (typehold->placeble() ==
                inventory[getpos(pointingto)]->placeble()) /// same class
            {
              ...
              *typehold = *typeholdcppy;
              ...
              typeholdcppy = inventory[getpos(pointingto)];
              if (typehold->placeble()) {
                inventory[getpos(pointingto)] =
                    new normalitems(typehold->type, typehold->number);
              } else {
                inventory[getpos(pointingto)] =
                    new nonplaceble(typehold->type, typehold->number);
                inventory[getpos(pointingto)]->setmaxnumber();
              }
              delete typehold;
              if (typeholdcppy->placeble()) {
                typehold =
                    new normalitems(typeholdcppy->type, typeholdcppy->number);

              } else {
                typehold =
                    new nonplaceble(typeholdcppy->type, typeholdcppy->number);
                typehold->setmaxnumber();
              }
            }
          }

        } else if (inventory[getpos(pointingto)]->type) /// take
        {
      ...
      if (rightreleased && getpos(pointingto) != -1 &&
        ...
        if (inventory[getpos(pointingto)]->type && !typehold->type) /// take
        {
          ...
          if (a) {
            ...
            } else /// take item dreapta
            {
              typehold = inventory[getpos(pointingto)];
              inventory[getpos(pointingto)] =
                  new nonplaceble(inventory[getpos(pointingto)]->type,
                                  inventory[getpos(pointingto)]->number -= a);
              typehold->number = a;
              typehold->setmaxnumber();
            }
        ...
        } else if (typehold->type &&
                   typehold->type == inventory[getpos(pointingto)]->type &&
                   typehold->placeble() ==
                       inventory[getpos(pointingto)]->placeble() &&
                   getpos(pointingto) != -2) /// place
        {
          typehold->number--;
          inventory[getpos(pointingto)]->number++;
          inventory[getpos(pointingto)]->setmaxnumber();
        ...
        } else if (typehold->type && inventory[getpos(pointingto)]->type == 0 &&
                   getpos(pointingto) != -2) /// place over air  dreapta
        {
          typehold->number--;
      ...
      if (leftreleased) {
        if (typehold->type) /// place
        {
          ...
          if (typehold->type == chestbuffer[getpos(pointingto)]->type &&
              typehold->placeble() ==
                  chestbuffer[getpos(pointingto)]->placeble() &&
              chestbuffer[getpos(pointingto)]->number !=
                  chestbuffer[getpos(pointingto)]->maxnumber) {
            if (chestbuffer[getpos(pointingto)]->number + typehold->number <=
                chestbuffer[getpos(pointingto)]->maxnumber) {
              chestbuffer[getpos(pointingto)]->number += typehold->number;
              delete typehold;
              typehold = new normalitems();
            } else {
              typehold->number -= (typehold->maxnumber -
                                   chestbuffer[getpos(pointingto)]->number);
              chestbuffer[getpos(pointingto)]->number =
                  chestbuffer[getpos(pointingto)]->maxnumber; /// when filled
            }

          } else /// change
          {
            if (typehold->placeble() ==
                chestbuffer[getpos(pointingto)]->placeble()) /// same class
            {
        ...
        } else if (chestbuffer[getpos(pointingto)]->type) /// take
        {
      ...
      if (rightreleased && getpos(pointingto) != -1 &&
        ...
        if (chestbuffer[getpos(pointingto)]->type && !typehold->type) /// take
        {
          ...
          if (a) {
            if (chestbuffer[getpos(pointingto)]
                    ->placeble()) /// take block dreapta
            {
              *typehold = *chestbuffer[getpos(pointingto)];
            ...
            } else /// take item dreapta
            {
              typehold = chestbuffer[getpos(pointingto)];
              chestbuffer[getpos(pointingto)] =
                  new nonplaceble(chestbuffer[getpos(pointingto)]->type,
                                  chestbuffer[getpos(pointingto)]->number -= a);
              typehold->number = a;
              typehold->setmaxnumber();
            }
                   ...
                   typehold->type == chestbuffer[getpos(pointingto)]->type &&
                   typehold->placeble() ==
                       chestbuffer[getpos(pointingto)]->placeble() &&
                   getpos(pointingto) != -2) /// place
        {
          ...
          if (typehold->number <= 0) {
            ...
            typehold = new normalitems();
                   ...
                   chestbuffer[getpos(pointingto)]->type == 0 &&
                   getpos(pointingto) != -2) /// place over air  dreapta
        {
          typehold->number--;
          ...
          if (typehold->placeble()) {
            // delete chestbuffer[getpos(pointingto)];
            chestbuffer[getpos(pointingto)] =
                new normalitems(typehold->type, 1);
            //*chestbuffer[getpos(pointingto)]= *typehold;
            chestbuffer[getpos(pointingto)]->setmaxnumber();
          } else {
            delete chestbuffer[getpos(pointingto)];
            chestbuffer[getpos(pointingto)] =
                new nonplaceble(typehold->type, 1);
            chestbuffer[getpos(pointingto)]->setmaxnumber();
          }
...
static int getpos(sf::Vector2i vect) {
  if (vect.y == 6 && vect.x == 0) {
    return -2;
  } // trash
  if (vect.y == -1 && vect.x == 1) {
    return -3;
  } // crafting
  if (vect.y == -1 && vect.x == 2) {
    return -4;
  } // chest
  if (vect.y == -1 && vect.x == 0) {
    return -5;
  } // inventory
  if (vect.y == 7 && vect.x == 4) {
    return -6;
  } // head
  if (vect.y == 8 && vect.x == 4) {
    return -7;
  } // body
  if (vect.y == 9 && vect.x == 4) {
    return -8;
  } // feet
