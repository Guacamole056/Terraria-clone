...
void entities::updatedraw() { /// !!!
  uint8_t numberofframes =
      7; // sprite.getTexture()->getSize().x/sizeofentity.x;
         /// seg fault !!!
  if (numberofframes != 1) {
...
bool entities::operator==(
  ...
  int intersectx =
      abs(deltax) - (other.sizeofentity.x / 2 + sizeofentity.x / 2);
  int intersecty =
      abs(deltay) - (other.sizeofentity.y / 2 + sizeofentity.y / 2);
