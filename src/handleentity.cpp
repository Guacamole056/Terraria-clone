...
void handleentity(entities &entity, float gravity) {
  ...
  if (entity.lastpos.x < entity.x) {
    entity.movingright = 1;
  } else {
    if (entity.lastpos.x > entity.x)
      entity.movingright = 0;
  }
  if (entity.lastpos.y < entity.y) {
    entity.movingdown = 1;
  } else {
    entity.movingdown = 0;
  }
  ...
  if (entity.movingdown) {
    ...
    for (int y = miny; y < maxy; y++) {
      ...
      for (int x = minx; x < maxx; x++) {
        if (world[x][y].family() == bl::platform_family &&
            !entity.down_pressed) {
          ...
          if (intersectx < 0.0f && intersecty < 0.0f &&
              entity.y + entity.sizeofentity.y < y * 32 + 28) {
            ...
            if (intersectx < 0.0f && intersecty < 0.0f) {
              if (abs(intersectx) <= abs(intersecty)) /// move on x
              {
                ...
                if (deltax < 0) {
                  ...
                  if (entity.movingright)
                    entity.x = x * 32 - entity.sizeofentity.x; /// dreapta
                }
  ...
  if (entity.x + entity.sizeofentity.x > 32 * Gamelenght) {
    entity.x = 32 * Gamelenght - entity.sizeofentity.x;
  }
  if (entity.y < 0) {
    entity.y = 0;
  }
  if (entity.y + entity.sizeofentity.y > 32 * Gameheight) {
    entity.y = 32 * Gameheight - entity.sizeofentity.y;
    entity.canjump = 1;
    checkfall(entity);
  }

  if (entity.y < entity.highestpoint) {
...
inline static int getminxb(entities &entity) {
  if (entity.x / 32 > 2) {
    return (entity.x / 32) - 2;
  }
  if (entity.x / 32 > 1) {
    return (entity.x / 32) - 1;
  } else {
    return 0;
  }
}
inline static int getminyb(entities &entity) {
  if (entity.y / 32 > 2) {
    return (entity.y / 32) - 2;
  }
  if (entity.y / 32 > 1) {
    return (entity.y / 32) - 1;
  } else {
    return 0;
  }
}
...
inline static int getmaxxb(entities &entity) {
  if (entity.x / 32 < Gamelenght - 4) {
    return (entity.x / 32) + 4;
  } else {
    return Gamelenght;
  }
  ...
  //  if(entity.x/32<Gamelenght-1){return (entity.x/32)+1;}else{return
  //  Gamelenght;}
}

inline static int getmaxyb(entities &entity) {

  if (entity.y / 32 < Gameheight - 4) {
    return (entity.y / 32) + 4;
  }
  if (entity.y / 32 < Gameheight - 3) {
    return (entity.y / 32) + 3;
  }
  if (entity.y / 32 < Gameheight - 2) {
    return (entity.y / 32) + 2;
  }
  if (entity.y / 32 < Gameheight - 1) {
    return (entity.y / 32) + 1;
  } else {
    return Gameheight;
  }
...
static void checkfall(entities &entity) {
  if (entity.y - entity.highestpoint > entity.falltolerance * 32) {
    ...
    if (entity.dealfalldamage(
            (entity.y - entity.highestpoint - entity.falltolerance * 32) / 4)) {
