...
menupage::menupage(sf::Texture texture, sf::RenderWindow &window,
                   sf::Vector2i position_, sf::Vector2i size_,
                   sf::Vector2i croppos1_, sf::Vector2i croppos2_ = {-1, -1}) {
  create(texture, window, position_, size_, croppos1_, croppos2_);
}

void menupage::create(sf::Texture texture, sf::RenderWindow &window,
                      sf::Vector2i position_, sf::Vector2i size_,
                      sf::Vector2i croppos1_,
                      sf::Vector2i croppos2_ = {-1, -1}) {
  if (croppos2_.x == -1) {
...
void menupage::update() {
  ...
  menu_sprite.setTextureRect(
      sf::IntRect(0, 0, croppos1.x, croppos1.y)); // draw left upper corner
  ...
  while (1) {
    if (sizex <= size.x - croppos2.x) {
      ...
      menu_sprite.setTextureRect(
          sf::IntRect(croppos2.x, 0, size.x, croppos1.y));
      window.draw(menu_sprite);
      break;
    } else {
