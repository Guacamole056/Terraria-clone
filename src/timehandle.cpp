...
void timehandle() {
  if (difficulty) {
    if (spawnwaittime < 0) {
      ...
      if (nrofenemies < 10) {
        ...
        if (difficulty == 2 && randompercent(50)) {
          spawnmonster();
        }
      }
  ...
  if (myclock.getElapsedTime().asSeconds() > whaittime) {
    ...
    // Create a thread with a shared_ptr to control its lifetime
    static std::shared_ptr<std::thread> updateThread;

    // If a previous thread exists and is joinable, join it
    if (updateThread && updateThread->joinable()) {
      updateThread->join();
    }

    // Create a new thread
    updateThread = std::make_shared<std::thread>(updatemap);
    updateThread->detach();
  }
}
...
void updatemap() {
  ...
  for (int x = 0; x < Gamelenght; x++) {
    for (int y = 0; y < Gameheight; y++) {
      if (world[x][y].type == bl::type::dirt && y > 0 &&
          (world[x][y - 1].type == 0 ||
           getfamilya(world[x][y - 1].type) != bl::blocy)) /// grass
      {
        if ((x > 0) && (world[x - 1][y].type == bl::grass) ||
            (x < Gamelenght) && (world[x + 1][y].type == bl::grass) ||
            (x > 0) && (y < Gameheight) &&
                (world[x - 1][y + 1].type == bl::grass) ||
            (x < Gamelenght) && (y < Gameheight) &&
                (world[x + 1][y + 1].type == bl::grass) ||
            (x > 0) && (y > 0) && (world[x - 1][y - 1].type == bl::grass) ||
            (x < Gamelenght) && (y > 0) &&
                (world[x + 1][y - 1].type == bl::grass) ||
            (x > 0) && (y < Gameheight - 1) &&
                (world[x - 1][y + 2].type == bl::grass) ||
            (x < Gamelenght) && (y < Gameheight - 1) &&
                (world[x + 1][y + 2].type == bl::grass) ||
      ...
      } else if (world[x][y].type == bl::grass && y > 0 &&
                 (world[x][y - 1].type == 0)) {
        if (randompercent(1)) {
          ...
          x++;
                 ...
                 getfamilya(world[x][y - 1].type) == bl::blocy) {
        if (randompercent(30))
          world[x][y].type = bl::dirt;
      } else if (world[x][y].type == bl::sappling) {
        if (randompercent(25) && (y > 0 && world[x][y - 1].type == 0) &&
